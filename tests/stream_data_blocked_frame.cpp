...
TEST(StreamDataBlockedFrameTest, EncodeDecode) {
    using namespace Packets;
    using namespace zclp_test_heplers;
    using namespace Frames;

    for (int i = 0; i < 100; i++) {
        StreamDataBlocked _in, _out;

        _in.stream_id = getRandomValidValue();
        _in.stream_data_limit = getRandomValidValue();
        ...
        ASSERT_EQ(_in.stream_data_limit, _out.stream_data_limit);
